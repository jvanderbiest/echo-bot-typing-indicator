...
namespace EchoBot
{
    ...
    /// </summary>
    public class CustomShowTypingMiddleware : IMiddleware
    {
        ...
        public CustomShowTypingMiddleware(int delay = 500, int period = 2000)
        {
            if (delay < 0)
            {
        ...
        private static async Task SendTypingAsync(ITurnContext turnContext, TimeSpan delay, TimeSpan period, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    if (!cancellationToken.IsCancellationRequested && !turnContext.Responded)
                    {
